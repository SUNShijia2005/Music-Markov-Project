...
def show_transition_matrix(parser, top_n=12):
    ...
    print(f"\n{'[Transition matrix of Bach BWV 26.6]':^80}")
    header = "从 \ 到".ljust(18)
    for s in top_states:
        label = f"{s[0]}/{s[1][0]}"
        header += f"{label:>10}"
...
def draw_heatmap(parser, top_n=20):
    ...
    colors = ['#FFFFFF', '#EDB2BF', '#E38B9F', '#C95F7A']
    custom_pink_cmap = LinearSegmentedColormap.from_list('custom_pink', colors, N=256)
    ...
    save_path = os.path.join(desktop_path, "Bach_BWV_26_6.png")

    print(f"正在尝试保存图片到: {save_path}")
    plt.savefig(save_path, dpi=300)
    plt.savefig("Bach_BWV_26_6.png", dpi=300)
