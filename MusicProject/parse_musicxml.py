...
def print_top_matrix(parser, top_n=10):
    # Find the top N states with the highest frequency
    ...
    for state in top_states:
        state_str = str(state[0])[:8]
        header += f"{state_str:>10}"
    print(header)
    print("-" * len(header))

    for state_from in top_states:
        state_from_idx = parser.states.index(state_from)
        row_str = f"{str(state_from[0])[:14]:<15}|"

        for state_to in top_states:
            state_to_idx = parser.states.index(state_to)
            row_data = parser.normalized_transition_probability_matrix[state_from_idx]
            prob = row_data[state_to_idx] - (row_data[state_to_idx - 1] if state_to_idx > 0 else 0)
            row_str += f"{prob:10.2f}"
        print(row_str)
